/**
 * Minimal inline Material Design icon set.
 *
 * The dashboard ships no icon library and renders inline `<svg>` everywhere, so
 * instead of adding a dependency (or an icon font) we keep the handful of
 * Material Design (filled, 24×24) paths we need right here. Icons inherit the
 * surrounding text colour via `fill-current`.
 */
...
const PATHS: Record<MaterialIconName, string> = {
  // https://fonts.google.com/icons?selected=Material+Icons:mic
  ...
  graphic_eq: 'M7 18h2V6H7v12zm4 4h2V2h-2v20zm-8-8h2v-4H3v4zm12 4h2V6h-2v12zm4-10v4h2v-4h-2z',
  ...
  // Terminal window with a prompt: Material-style geometry for a card glyph.
  terminal: 'M20 4H4a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2zm0 14H4V6h16v12zM7.5 9.2 6.44 10.26 8.68 12.5l-2.24 2.24 1.06 1.06 3.3-3.3-3.3-3.3zM12.5 14.4h4.5V16h-4.5z',
  pause: 'M6 19h4V5H6v14zm8-14v14h4V5h-4z',
}

export interface MaterialIconProps {
  name: MaterialIconName
  /** Tailwind size/colour classes; defaults to a 1rem icon using the current text colour. */
  className?: string
  /** Spin continuously (for “working” icons such as `sync`). */
  spin?: boolean
}

export default MaterialIcon

export function MaterialIcon({ name, className = 'h-4 w-4', spin = false }: MaterialIconProps) {
  return (
    <svg
      viewBox="0 0 24 24"
      aria-hidden="true"
      focusable="false"
      className={`fill-current${spin ? ' animate-spin' : ''}${className ? ` ${className}` : ''}`}
    >
      <path d={PATHS[name]} />
    </svg>
  )
