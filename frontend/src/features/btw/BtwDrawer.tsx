  ...
  return <div className="fixed inset-y-0 right-0 z-50 w-full md:w-[min(680px,70vw)] bg-bg border-l border-border shadow-2xl flex flex-col">
    ...
    <footer className="border-t border-border p-3">
      <div className="flex gap-2">
        ...
        {snapshot?.status === 'busy' ? <button className="px-3 rounded-lg border border-danger/40 text-danger" onClick={() => void send({ type: 'abort' })}>Stop</button> : <button className="px-4 rounded-lg bg-accent text-white disabled:opacity-40" disabled={!input.trim() || !attached} onClick={submit}>Send</button>}
      </div>
