...
export const liveSessionApi = {
   ...
   */
  sessionTree: (file: string, detail: 'collapsed' | 'full' = 'collapsed') =>
    fetch(`/api/session-tree?file=${encodeURIComponent(file)}${detail === 'full' ? '&detail=full' : ''}`, { credentials: 'same-origin' })
