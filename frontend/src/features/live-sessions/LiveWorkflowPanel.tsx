...
export default function LiveWorkflowPanel({ workflow, sessions, onClose, onOpenSubagent }: LiveWorkflowPanelProps) {
  ...
  return <aside className="fixed inset-y-0 right-0 z-40 flex w-[min(680px,94vw)] flex-col border-l border-accent/30 bg-bg shadow-2xl shadow-black/40">
    <header className="shrink-0 border-b border-border bg-card px-4 py-3">
      <div className="flex items-start gap-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2"><span className={`text-sm ${statusClass(workflowStatus)}`}>{statusIcon(workflowStatus)}</span><span className="rounded bg-accent/15 px-1.5 py-0.5 text-2xs text-accent">Workflow</span><h2 className="min-w-0 flex-1 truncate text-sm font-semibold text-text-strong">{text(workflow.label, text(workflow.id, '未命名 Workflow'))}</h2><span className={`text-2xs ${statusClass(workflowStatus)}`}>{statusLabel(workflowStatus)}</span></div>
    ...
    <div className="min-h-0 flex-1 overflow-y-auto p-3 space-y-3">
      ...
      {linkedSessions.length > 0 && <section className="rounded-lg border border-border bg-card p-3"><div className="mb-2 text-2xs font-medium text-text-strong">关联子 Agent · {linkedSessions.length}</div><div className="space-y-1.5">{linkedSessions.map(session => <button key={session.processInstanceId} type="button" onClick={() => onOpenSubagent(session.processInstanceId)} className="flex w-full items-center gap-2 rounded border border-border/70 bg-bg/40 px-2 py-1.5 text-left hover:border-accent"><span className={`h-1.5 w-1.5 rounded-full ${session.status === 'running' ? 'bg-accent animate-pulse' : 'bg-muted'}`} /><span className="min-w-0 flex-1 truncate text-2xs text-text-strong">{session.sessionName || `PID ${session.pid}`}</span><span className="text-2xs text-muted">{session.status}</span></button>)}</div></section>}
      {error && <section className="rounded-lg border border-danger/30 bg-danger-subtle p-3"><div className="mb-1 text-2xs font-medium text-danger">失败原因</div><MarkdownRenderer content={error} /></section>}
    </div>
